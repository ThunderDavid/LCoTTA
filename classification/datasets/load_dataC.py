...
def load_imagenetc(
    ...
    x_test, y_test, name, paths = next(iter(test_loader))
...
def load_imagenetC(
   n_examples: Optional[int] = 5000,
    ...
    indices = np.linspace(0, total_num - 1, n_examples, dtype=int)
    imagenet_subset = Subset(imagenet, indices)

    test_loader = data.DataLoader(imagenet_subset,
                                  batch_size=batch_size,
                                  shuffle=shuffle,
                                  num_workers=num_of_workers)
    return test_loader
