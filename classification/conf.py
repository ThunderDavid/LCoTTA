...
_C.DEYO.AUG_TYPE = "pixel"          # Choose from: ['occ', 'patch', 'pixel']
...
_C.SUBSPACE.W_NUM = 100
_C.SUBSPACE.N_COMPONENTS = 20
_C.SUBSPACE.BATCH_STEP = 50
...
def load_cfg_from_args(description="Config options."):
    ...
    parser.add_argument('--tag', default='None', type=str, help='the tag of experiment')
    ...
    if args.tag != 'None':
        log_dest = log_dest.split('.txt')[0] + '_' + args.tag + '.txt'

    cfg.SAVE_DIR = os.path.join(cfg.SAVE_DIR, f"{cfg.MODEL.ARCH}", f"{cfg.MODEL.ADAPTATION}_{cfg.CORRUPTION.DATASET}")
    #f"_{cfg.CORRUPTION.DATASET}_{current_time}")
