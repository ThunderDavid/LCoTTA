...
def get_torchvision_model(model_name: str, weight_version: str = "IMAGENET1K_V1"):
    ...
    """
    # Ensure torchvision version is >= 0.13
    ...
    try:
        ...
        # Handle older versions where list_models is not available
        available_models = [name for name in dir(torchvision.models) if callable(getattr(torchvision.models, name))]

    if model_name not in available_models:
        raise ValueError(f"Model '{model_name}' is not available in torchvision. Choose from: {available_models}")

    try:
        # Get the weight object of the specified model
        ...
        raise ValueError(f"Model '{model_name}' does not support weight versioning in your current torchvision version.")

    # Check if the specified weight type is available
    if weight_version not in available_weights:
        raise ValueError(f"Weight type '{weight_version}' is not supported for model '{model_name}'. "
    ...
    # Get the transformation and add normalization
...
def normalize_model(model, mean, std):
    """A placeholder normalization function."""
...
class ZeroShotCLIP(nn.Module):
    ...
    def forward(self, imgs_test, return_features=False):
        ...
        # cosine subspace_grad as logits
...
def get_model(cfg, num_classes: int, device: Union[str, torch.device]):
    ...
    elif cfg.MODEL.ARCH == 'ViT':
        base_model = timm.create_model('vit_base_patch16_224', pretrained=True).cuda()
