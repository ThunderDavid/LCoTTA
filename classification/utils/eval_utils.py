...
#         for i, data in enumerate(data_loader):
#             imgs, labels = data[0], data[1]
#             outputs = model([img.to(device) for img in imgs], labels.to(device)) if isinstance(imgs, list) else model(imgs.to(device), labels.to(device))
#             if len(outputs) > 1:
#                 output, outputs_test, outputs_ema = outputs
#                 if outputs_test is not None:
#                     predictions_stu = outputs_test.argmax(1)
#                     predictions_tea = outputs_ema.argmax(1)
#             else:
#                 output = outputs
#                 outputs_test = None
#             predictions = output.argmax(1)
...
#             num_correct += (predictions == labels.to(device)).float().sum()
#             if outputs_test is not None:
#                 correct_stu += (predictions_stu == labels.to(device)).float().sum()
#                 correct_tea += (predictions_tea == labels.to(device)).float().sum()
#
#             if "mixed_domains" in setting and len(data) >= 3:
#                 domain_dict = split_results_by_domain(domain_dict, data, predictions)
#
...
#     acc.append(accuracy)
#     if outputs_test is not None:
#         accuracy_stu = correct_stu.item() / len(data_loader.dataset)
#         accuracy_tea = correct_tea.item() / len(data_loader.dataset)
#         acc.append(accuracy_stu)
#         acc.append(accuracy_tea)
#     return acc, domain_dict, num_samples