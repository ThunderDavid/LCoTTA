...
from datasets.load_dataC import *
from datasets.data_loading import get_transform
    ...
    for repeat in range(50):
        ...
        for i_dom, domain_name in enumerate(domain_seq_loop):
            ...
            for severity in severities:
                transform = get_transform(cfg.CORRUPTION.DATASET, cfg.MODEL.ADAPTATION, None,  cfg.MODEL.USE_CLIP, cfg.TEST.N_AUGMENTATIONS)
                test_data_loader = load_imagenetC(5000, severity,cfg.DATA_DIR,False, [domain_name],cfg.num_workers, transform,cfg.TEST.BATCH_SIZE)
        ...
        logger.info(
            f"Repeat {repeat}, the error is {np.mean(repeat_err):.2%}, accuracy is {np.mean(repeat_acc):.2%}, duration is {duration:.2f} seconds \n"
        )
    if len(errs_5) > 0:
